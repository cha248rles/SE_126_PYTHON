...
#--------FUNCTIONS--------------------------------------------
def again(): #<--- Func header
    '''this func asks a user if they would like to enter another temp checks the response for validity and trhen returns a valid response to main program '''
    ans = input("\t\tWould you like to enter another temperature? [y/n]: ".lower())

    #while loop trap- checks for valid input 
    ...
    return ans
def converter(f):
    '''This func is passed a temp f value ('f' parameter), converts to celsius, and return the tempC value'''
    c = (f - 32) * (5 / 9)
    return c
...
while answer == "y" :
    ...
    temp_count = temp_count +1 #tmep_count
    temp_total += tempF
    ...
    answer = again() # value from func is getting used 
